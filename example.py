...
def main():
    ...
    gamma = 0.9
    start = time.time()
    policy, value_func, s, num_iter = rl.policy_iteration_sync(env, gamma, max_iterations=int(1e4), tol=1e-3)
    duration = time.time()-start
    #print('Agent received total reward of: %f' % total_reward)
    print('Agent took %d number of policy improvement iterations' % s)
    print('Agent took %d number of value iterations' % num_iter)
    print('Run time took %f seconds' % duration)
    env.render()
